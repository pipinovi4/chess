...
function addCustomMethods(socket, gameModel) {
    socket.receiveGameData = async (roomId, gameId) => {
        try {
            ...
            console.error('Error when trying to get game data in the custom socket method:', error);
            throw new Error('Error when trying to get game data in the custom socket method.');
        }
    };
